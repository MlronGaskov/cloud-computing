(ns integration.java-functional-object-test
  (:require [clojure.test :refer [deftest is use-fixtures]]
            [cloud.remote :as remote])
  (:import (demo.fn Times2Function DivisibleBy3Predicate SumBinaryOperator)
...
(deftest serializable-java-function-object-works-remotely
  (let [f (Times2Function.)]
    (is (= 10 (remote-java-function-test f 5)))))

(deftest serializable-java-predicate-object-works-remotely
  (let [p (DivisibleBy3Predicate.)]
    (is (= true (remote-java-predicate-test p 6)))
    (is (= false (remote-java-predicate-test p 8)))))

(deftest serializable-java-binop-object-works-remotely
  (let [op (SumBinaryOperator.)]
